...
{
    [BasicAuthentication]
    public class CustomerGetByRangV2Controller : ApiController
    {
        // GET api/<controller>
        //public IEnumerable<CustomerModel> Post([FromUri]string api_name, string api_key,[FromBody]string json)
        public string Post([FromUri]string p, [FromUri]string msgid, [FromBody]string json)//json={"UserName":"02726","criteriaValue":"1,10"}
        {
            Class1 c = new Class1();
            string ERR = "Succeed", SMS = "", ExSMS = "", ERRCode = "";
            string ServerDate = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff");
            string ControllerName = "CustomerGetByRangV2";
            ...
            try
            {
                ...
                #region read json
                CustomerGetByRangRQModelV2 jObj = null;
                string criteriaValue = "", UserName = "";
                if (ERR != "Error")
                {
                    try
                    {
                        jObj = JsonConvert.DeserializeObject<CustomerGetByRangRQModelV2>(json);
                ...
                if (ERR != "Error")
                {
                    ...
                    DataTable dt = c.ReturnDT("exec T24_GetCustomerByAMOrCOByDeviceByRangV2 @UserName='" + UserName + "',@criteriaValue='"+ criteriaValue + "'");
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        data.imgUrl = dt.Rows[i]["imgUrl"].ToString();
                        DataList.Add(data);
                        #endregion params
                    }
            ...
            try
            {
                ...
                c.T24_AddLog(FileNameForLog, "RS", "", ControllerName);
            }
            catch { }
            return RSDataStr;
        }

...
{
    [BasicAuthentication]
    public class HolidayGetByTabController : ApiController
    {
        ...
        public string Post([FromUri]string p, [FromUri]string msgid, [FromBody]string json)//new-> {"LatestDateTime":"yyyy-MM-dd HH:mm:ss.fff"} | old->{"FYear":"2025","TYear":"2025"}->ED
        {
            Class1 c = new Class1();
            string ERR = "Succeed", SMS = "", ExSMS = "", ERRCode = "";
            string ServerDate = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff");
            string ControllerName = "HolidayGetByTab";
            string FileNameForLog = msgid + "_" + ServerDate.Replace("-", "_").Replace(" ", "_").Replace(":", "_").Replace(".", "_");
            List<HolidayGetByTabRSModel> RSData = new List<HolidayGetByTabRSModel>();
            HolidayGetByTabRSModel ListHeader = new HolidayGetByTabRSModel();
            try
            {
                #region msgid
                if (ERR != "Error")
                ...
                #region read json
                HolidayGetByTabRQ jObj = null;
                string LatestDateTime = "";
                if (ERR != "Error")
                {
                    try
                    {
                        jObj = JsonConvert.DeserializeObject<HolidayGetByTabRQ>(json);
                ...
                if (ERR != "Error")
                {
                    List<HolidayGetByTabRSList> DataList = new List<HolidayGetByTabRSList>();
                    string sql = "exec sp_HolidayGetFromTab @LatestDateTime='" + LatestDateTime + "'";
                    DataTable dt = c.ReturnDT(sql);
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        #region params
                        HolidayGetByTabRSList data = new HolidayGetByTabRSList();
    ...
    public class HolidayGetByTabRQ
    {
        public string LatestDateTime { get; set; }
        //public string FYear { get; set; }
        //public string TYear { get; set; }
    }
    public class HolidayGetByTabRSModel
    {
        public string ERR { get; set; }
        public string SMS { get; set; }
        public string ERRCode { get; set; }
        public List<HolidayGetByTabRSList> DataList { get; set; }
    }
    public class HolidayGetByTabRSList
    {

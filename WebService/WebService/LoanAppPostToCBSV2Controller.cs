...
{
    public class LoanAppPostToCBSV2Controller : ApiController
    {        
        // POST api/<controller>
        public IEnumerable<LoanAppPostToCBSResModel> Post([FromUri]string api_name, string api_key, string loanappid)
        {
            ...
            try {
                ...
                for (int i1 = 0; i1 < dt1.Rows.Count; i1++)
                {
                    if (ERR != "Error") {
                        try
                        {
                            ...
                            fileHeader = "LoanAppPostToCBSV2_InstID_" + InstID+ "_LID_" + LoanAppID + "_" + ServerDate;
                            //old version -> AddEditCustomerV2 | new verion -> AddEditCustomerV3
                            string[] rs=null;
                            //if (Convert.ToInt32(LoanAppPersonID) <= 5194708)//need to be change at release time 'All branch : 5194708 and 'KSV : 2516639
                            //{
                            //    rs = c.AddEditCustomerV2(fileHeader, LoanAppPersonID, LoanAppPersonTypeID);
                            //}
                            //else {
                            //    rs = c.AddEditCustomerV3(fileHeader, LoanAppPersonID, LoanAppPersonTypeID);
                            //}
                ...
                {
                    //add loan
                    string[] rs = c.AddLoanV2(fileHeader, LoanAppID, CUSTOMERID, api_name);
                    ERR = rs[0];
                    SMS = rs[1];

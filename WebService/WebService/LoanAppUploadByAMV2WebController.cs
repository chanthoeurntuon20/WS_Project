...
{
    [BasicAuthentication]
    public class LoanAppUploadByAMV2WebController : ApiController
    {
        public IEnumerable<LoanAppUploadByAMV2RS> Post([FromUri]string api_name, string api_key, string username, [FromBody]string json) {
            ...
            string ControllerName = "LoanAppUploadByAMV2Web";
                                                ...
                                                if (isAllow == "allow")
                                                {
                                                    ...
                                                    if (ERR != "Error")
                                                    {
                                                        ...
                                                        try
                                                        {
                                                            ...
                                                            string xmlCreateSaving = "", xmlCheckSaving = "";
                                                            if (ZeroFailOneSucceed == "1")
                                                            {
                                                                ...
                                                                string url = c.GetTabletWSUrl() + "/api/AccountCreationToCBS?api_name=" + c.GetTabletWSAPIName() 
                                                                    + "&api_key=" + c.GetTabletWSAPIKey() + "&type=json&json=" + jsonWS;
                                                                xmlCreateSaving = url;

                                                                string jsonWSCheck = "[{\"user\":\"none\",\"pwd\":\"none\",\"device_id\":\"none\",\"app_vName\":\"none\""
                                                                    + ",\"CreCompany\":\"" + CreCompany + "\""
                                                                    + ",\"CustomerID\":\"" + CustomerID + "\""
                                                                    + ",\"CATEGORY\":\"6001\""
                                                                    + ",\"ACCOUNTTITLE1\":\"None\""
                                                                    + ",\"SHORTTITLE\":\"None\""
                                                                    + ",\"LoanCurrency\":\"" + LoanCurrency + "\"}]";
                                                                string urlCheck = c.GetTabletWSUrl() + "/api/AccountGetFromCBS?api_name=" + c.GetTabletWSAPIName() 
                                                                    + "&api_key=" + c.GetTabletWSAPIKey() + "&type=json&json=" + jsonWSCheck;
                                                                xmlCheckSaving = urlCheck;
                                                            ...
                                                            Com1.Parameters.AddWithValue("@xmlCheckSaving", xmlCheckSaving);
                                                            Com1.ExecuteNonQuery();
                                                            Con1.Close();
                                                        }
                                                        catch (Exception ex)

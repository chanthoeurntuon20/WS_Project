...
{
    [BasicAuthentication]
    public class LoanOverdueLogsGetController : ApiController
    {
        // GET api/<controller>
        public JsonResponse<LoanOverdueLogsModel> Get(string api_name, string api_key, string json)//json=[{"LoanAcc":"none"}]
        {
            ...
            try
            {
                ...
                if (ERR != "Error")
                {
                    ...
                    DataTable dt = c.ReturnDT("exec T24_GetLoanOverdueLogs @LoanAcc='" + LoanAcc + "',@PageNumber=" + page + ",@PageSize=" + limit + "");

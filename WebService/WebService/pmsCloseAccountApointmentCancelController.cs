...
{
    [BasicAuthentication]
    public class pmsCloseAccountApointmentCancelController : ApiController
    {
        ...
        public string Post([FromUri]string p, [FromUri]string msgid, [FromBody]string json)
        {
            ...
            string ControllerName = "pmsCloseAccountApointmentCancel";
            string FileNameForLog = msgid + "_" + ServerDate.Replace("-", "_").Replace(" ", "_").Replace(":", "_").Replace(".", "_");
            List<pmsCloseAccountApointmentCancelRSModel> RSData = new List<pmsCloseAccountApointmentCancelRSModel>();
            try
            {
                #region msgid
                if (ERR != "Error")
                ...
                #region read json
                pmsCloseAccountApointmentCancelRQModel jObj = null;
                string
                    pms_apo_id = "", CID = "", comment_cancel = "", create_by = "";

                if (ERR != "Error")
                {
                    try
                    {
                        jObj = JsonConvert.DeserializeObject<pmsCloseAccountApointmentCancelRQModel>(json);

                        pms_apo_id = jObj.pms_apo_id;
                        CID = jObj.CID;
                        comment_cancel = jObj.comment_cancel;
                ...
                if (ERR != "Error")
                {
                    ...
                    try
                    {
                        string sql = "exec fact_pms_apointment_account_close_cancel @pms_apo_id,@CID,@comment_cancel,@create_by";
                        Com1.CommandText = sql;
                        Com1.Parameters.Clear();
                        Com1.Parameters.AddWithValue("@pms_apo_id", pms_apo_id);
                        Com1.Parameters.AddWithValue("@CID", CID);
                        Com1.Parameters.AddWithValue("@comment_cancel", comment_cancel);
            ...
            pmsCloseAccountApointmentCancelRSModel Header = new pmsCloseAccountApointmentCancelRSModel();
            Header.ERR = ERR;
            Header.SMS = SMS;
...
public class pmsCloseAccountApointmentCancelRQModel
{
...
public class pmsCloseAccountApointmentCancelRSModel
{

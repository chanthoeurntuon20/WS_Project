...
{
    [BasicAuthentication]
    public class pmsCloseAccountApointmentMeetController : ApiController
    {
        ...
        public string Post([FromUri]string p, [FromUri]string msgid, [FromBody]string json)
        {
            ...
            string ControllerName = "pmsCloseAccountApointmentMeet";
            string FileNameForLog = msgid + "_" + ServerDate.Replace("-", "_").Replace(" ", "_").Replace(":", "_").Replace(".", "_");
            List<pmsCloseAccountApointmentMeetRSModel> RSData = new List<pmsCloseAccountApointmentMeetRSModel>();
            try
            {
                #region msgid
                if (ERR != "Error")
                ...
                #region read json
                pmsCloseAccountApointmentMeetRQModel jObj = null;
                string
                ...
                if (ERR != "Error")
                {
                    try
                    {
                        jObj = JsonConvert.DeserializeObject<pmsCloseAccountApointmentMeetRQModel>(json);
                ...
                if (ERR != "Error")
                {
                    ...
                    try
                    {
                        string sql = "exec fact_pms_apointment_account_close_meet @pms_apo_id,@CID,@pms_result,@pms_reason,@comment_meet,@create_by";
            ...
            pmsCloseAccountApointmentMeetRSModel Header = new pmsCloseAccountApointmentMeetRSModel();
            Header.ERR = ERR;
            Header.SMS = SMS;
...
public class pmsCloseAccountApointmentMeetRQModel
{
...
public class pmsCloseAccountApointmentMeetRSModel
{

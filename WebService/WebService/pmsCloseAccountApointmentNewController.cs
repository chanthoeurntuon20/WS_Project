...
{
    [BasicAuthentication]
    public class pmsCloseAccountApointmentNewController : ApiController
    {
        ...
        public string Post([FromUri]string p, [FromUri]string msgid, [FromBody]string json)
        {
            ...
            string ControllerName = "pmsCloseAccountApointmentNew";
            string FileNameForLog = msgid + "_" + ServerDate.Replace("-", "_").Replace(" ", "_").Replace(":", "_").Replace(".", "_");
            List<pmsCloseAccountApointmentNewRSModel> RSData = new List<pmsCloseAccountApointmentNewRSModel>();
            try
            {
                #region msgid
                if (ERR != "Error")
                ...
                #region read json
                pmsCloseAccountApointmentNewRQModel jObj = null;
                string
                ...
                if (ERR != "Error")
                {
                    try
                    {
                        jObj = JsonConvert.DeserializeObject<pmsCloseAccountApointmentNewRQModel>(json);
                        CID = jObj.CID;
                        try
                        {
                ...
                if (ERR != "Error")
                {
                    ...
                    try
                    {
                        string sql = "exec fact_pms_apointment_account_close_new @CID,@date_meet,@comment_new,@create_by";
            ...
            pmsCloseAccountApointmentNewRSModel Header = new pmsCloseAccountApointmentNewRSModel();
            Header.ERR = ERR;
            Header.SMS = SMS;
...
public class pmsCloseAccountApointmentNewRQModel
{
...
public class pmsCloseAccountApointmentNewRSModel
{

...
{
    [BasicAuthentication]
    public class pmsDueBalanceApointmentCancelController : ApiController
    {
        public string Post([FromUri]string p, [FromUri]string msgid, [FromBody]string json)
        {
            ...
            string ControllerName = "pmsDueBalanceApointmentCancel";
            string FileNameForLog = msgid + "_" + ServerDate.Replace("-", "_").Replace(" ", "_").Replace(":", "_").Replace(".", "_");
            List<pmsDueBalanceApointmentCancelRSModel> RSData = new List<pmsDueBalanceApointmentCancelRSModel>();
            try
            {
                #region msgid
                if (ERR != "Error")
                ...
                #region read json
                pmsDueBalanceApointmentCancelRQModel jObj = null;
                string
                ...
                if (ERR != "Error")
                {
                    try
                    {
                        jObj = JsonConvert.DeserializeObject<pmsDueBalanceApointmentCancelRQModel>(json);
                ...
                if (ERR != "Error")
                {
                    ...
                    try
                    {
                        string sql = "exec fact_pms_apointment_due_balance_cancel @pms_apo_id,@CID,@comment_cancel,@create_by";
            ...
            pmsDueBalanceApointmentCancelRSModel Header = new pmsDueBalanceApointmentCancelRSModel();
            Header.ERR = ERR;
            Header.SMS = SMS;
...
public class pmsDueBalanceApointmentCancelRQModel
{
...
public class pmsDueBalanceApointmentCancelRSModel
{

...
{
    [BasicAuthentication]
    public class pmsDueBalanceApointmentMeetController : ApiController
    {
        public string Post([FromUri]string p, [FromUri]string msgid, [FromBody]string json)
        {
            ...
            string ControllerName = "pmsDueBalanceApointmentMeet";
            string FileNameForLog = msgid + "_" + ServerDate.Replace("-", "_").Replace(" ", "_").Replace(":", "_").Replace(".", "_");
            List<pmsDueBalanceApointmentMeetRSModel> RSData = new List<pmsDueBalanceApointmentMeetRSModel>();
            try
            {
                #region msgid
                if (ERR != "Error")
                ...
                #region read json
                pmsDueBalanceApointmentMeetRQModel jObj = null;
                string
                ...
                if (ERR != "Error")
                {
                    try
                    {
                        jObj = JsonConvert.DeserializeObject<pmsDueBalanceApointmentMeetRQModel>(json);
                ...
                if (ERR != "Error")
                {
                    ...
                    try
                    {
                        string sql = "exec fact_pms_apointment_due_balance_meet @pms_apo_id,@CID,@pms_result,@pms_reason,@comment_meet,@create_by";
            ...
            pmsDueBalanceApointmentMeetRSModel Header = new pmsDueBalanceApointmentMeetRSModel();
            Header.ERR = ERR;
            Header.SMS = SMS;
...
public class pmsDueBalanceApointmentMeetRQModel
{
...
public class pmsDueBalanceApointmentMeetRSModel
{

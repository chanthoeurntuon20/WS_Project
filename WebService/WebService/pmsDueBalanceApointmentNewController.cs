...
{
    [BasicAuthentication]
    public class pmsDueBalanceApointmentNewController : ApiController
    {
        ...
        public string Post([FromUri]string p, [FromUri]string msgid, [FromBody]string json)
        {
            ...
            string ControllerName = "pmsDueBalanceApointmentNew";
            string FileNameForLog = msgid + "_" + ServerDate.Replace("-", "_").Replace(" ", "_").Replace(":", "_").Replace(".", "_");
            List<pmsDueBalanceApointmentNewRSModel> RSData = new List<pmsDueBalanceApointmentNewRSModel>();
            try
            {
                #region msgid
                if (ERR != "Error")
                ...
                #region read json
                pmsDueBalanceApointmentNewRQModel jObj = null;
                string
                ...
                if (ERR != "Error")
                {
                    try
                    {
                        jObj = JsonConvert.DeserializeObject<pmsDueBalanceApointmentNewRQModel>(json);
                        CID = jObj.CID;
                        try
                        {
                ...
                if (ERR != "Error")
                {
                    ...
                    try
                    {
                        string sql = "exec fact_pms_apointment_due_balance_new @CID,@date_meet,@comment_new,@create_by";
            ...
            pmsDueBalanceApointmentNewRSModel Header = new pmsDueBalanceApointmentNewRSModel();
            Header.ERR = ERR;
            Header.SMS = SMS;
...
public class pmsDueBalanceApointmentNewRQModel
{
...
public class pmsDueBalanceApointmentNewRSModel
{

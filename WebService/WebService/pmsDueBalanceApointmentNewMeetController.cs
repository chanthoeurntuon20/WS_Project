...
{
    [BasicAuthentication]
    public class pmsDueBalanceApointmentNewMeetController : ApiController
    {
        ...
        public string Post([FromUri]string p, [FromUri]string msgid, [FromBody]string json)
        {
            ...
            string ControllerName = "pmsDueBalanceApointmentNewMeet";
            string FileNameForLog = msgid + "_" + ServerDate.Replace("-", "_").Replace(" ", "_").Replace(":", "_").Replace(".", "_");
            List<pmsDueBalanceApointmentNewMeetRSModel> RSData = new List<pmsDueBalanceApointmentNewMeetRSModel>();
            try
            {
                #region msgid
                if (ERR != "Error")
                ...
                #region read json
                pmsDueBalanceApointmentNewMeetRQModel jObj = null;
                string
                ...
                if (ERR != "Error")
                {
                    try
                    {
                        jObj = JsonConvert.DeserializeObject<pmsDueBalanceApointmentNewMeetRQModel>(json);
                ...
                if (ERR != "Error")
                {
                    ...
                    try
                    {
                        string sql = "exec fact_pms_apointment_due_balance_new_meet @pms_apo_id,@CID,@date_meet_new,@comment_meet_new,@create_by";
            ...
            pmsDueBalanceApointmentNewMeetRSModel Header = new pmsDueBalanceApointmentNewMeetRSModel();
            Header.ERR = ERR;
            Header.SMS = SMS;
...
public class pmsDueBalanceApointmentNewMeetRQModel
{
...
public class pmsDueBalanceApointmentNewMeetRSModel
{

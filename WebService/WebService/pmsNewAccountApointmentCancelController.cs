...
{
    [BasicAuthentication]
    public class pmsNewAccountApointmentCancelController : ApiController
    {
        public string Post([FromUri]string p, [FromUri]string msgid, [FromBody]string json)
        {
            ...
            string ControllerName = "pmsNewAccountApointmentCancel";
            string FileNameForLog = msgid + "_" + ServerDate.Replace("-", "_").Replace(" ", "_").Replace(":", "_").Replace(".", "_");
            List<pmsNewAccountApointmentCancelRSModel> RSData = new List<pmsNewAccountApointmentCancelRSModel>();
            try
            {
                #region msgid
                if (ERR != "Error")
                ...
                #region read json
                pmsNewAccountApointmentCancelRQModel jObj = null;
                string
                ...
                if (ERR != "Error")
                {
                    try
                    {
                        jObj = JsonConvert.DeserializeObject<pmsNewAccountApointmentCancelRQModel>(json);
                ...
                if (ERR != "Error")
                {
                    ...
                    try
                    {
                        string sql = "exec fact_pms_apointment_new_account_cancel @pms_apo_id,@CID,@comment_cancel,@create_by";
            ...
            pmsNewAccountApointmentCancelRSModel Header = new pmsNewAccountApointmentCancelRSModel();
            Header.ERR = ERR;
            Header.SMS = SMS;
...
public class pmsNewAccountApointmentCancelRQModel
{
...
public class pmsNewAccountApointmentCancelRSModel
{

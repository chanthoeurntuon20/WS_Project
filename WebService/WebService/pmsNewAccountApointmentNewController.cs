...
{
    [BasicAuthentication]
    public class pmsNewAccountApointmentNewController : ApiController
    {
        public string Post([FromUri]string p, [FromUri]string msgid, [FromBody]string json)
        {
            ...
            string ControllerName = "pmsNewAccountApointmentNew";
            string FileNameForLog = msgid + "_" + ServerDate.Replace("-", "_").Replace(" ", "_").Replace(":", "_").Replace(".", "_");
            List<pmsNewAccountApointmentNewRSModel> RSData = new List<pmsNewAccountApointmentNewRSModel>();
            try
            {
                #region msgid
                if (ERR != "Error")
                ...
                #region read json
                pmsNewAccountApointmentNewRQModel jObj = null;
                string
                ...
                if (ERR != "Error")
                {
                    try
                    {
                        jObj = JsonConvert.DeserializeObject<pmsNewAccountApointmentNewRQModel>(json);
                        CID = jObj.CID;
                        try
                        {
                ...
                if (ERR != "Error")
                {
                    ...
                    try
                    {
                        string sql = "exec fact_pms_apointment_new_account_new @CID,@date_meet,@comment_new,@create_by";
            ...
            pmsNewAccountApointmentNewRSModel Header = new pmsNewAccountApointmentNewRSModel();
            Header.ERR = ERR;
            Header.SMS = SMS;
...
public class pmsNewAccountApointmentNewRQModel
{
...
public class pmsNewAccountApointmentNewRSModel
{

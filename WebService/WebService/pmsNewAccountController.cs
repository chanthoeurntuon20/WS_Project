...
{
    [BasicAuthentication]
    public class pmsNewAccountController : ApiController
    {
        ...
        {//json={"search_type":"CID","search_data":"1614328","hit_date_from":"2020-08-21","hit_date_to":"2020-08-21","co_T24":"1234"}
            ...
            string ControllerName = "pmsNewAccount";
            string FileNameForLog = msgid + "_" + ServerDate.Replace("-", "_").Replace(" ", "_").Replace(":", "_").Replace(".", "_");
            List<pmsNewAccountRSModel> RSData = new List<pmsNewAccountRSModel>();
            try
            {
                #region msgid
                if (ERR != "Error")
                ...
                #region read json
                pmsNewAccountRQModel jObj = null;
                string search_type = "", search_data="", hit_date_from="", hit_date_to="", co_T24="";
                if (ERR != "Error")
                {
                    try
                    {
                        jObj = JsonConvert.DeserializeObject<pmsNewAccountRQModel>(json);
                        //pms_sp = jObj.pms_sp;
                ...
                if (ERR != "Error")
                {
                    ...
                    try {
                        string sql = "exec pmsamk_new_account @search_type=@search_type,@search_data=@search_data,@hit_date_from=@hit_date_from,@hit_date_to=@hit_date_to,@co_T24=@co_T24";
                        ...
                        pmsNewAccountRSModel ListHeader = new pmsNewAccountRSModel();
                        ListHeader.ERR = ERR;
                        ListHeader.SMS = SMS;
                        ListHeader.ERRCode = ERRCode;
                        List<pmsNewAccountDataRSModel> DataList = new List<pmsNewAccountDataRSModel>();
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            #region params
                            pmsNewAccountDataRSModel data = new pmsNewAccountDataRSModel();
                            ...
                            data.number_of_enquiry = dt.Rows[i]["New_Acount"].ToString();
                            DataList.Add(data);
                            #endregion params
                        }
            ...
            if (ERR == "Error")
            {
                pmsNewAccountRSModel CustHeader = new pmsNewAccountRSModel();
                CustHeader.ERR = ERR;
                CustHeader.SMS = SMS;
    ...
    #region model    
    public class pmsNewAccountRQModel
    {
    ...
    public class pmsNewAccountRSModel
    {
        public string ERR { get; set; }
        public string SMS { get; set; }
        public string ERRCode { get; set; }
        public List<pmsNewAccountDataRSModel> Data { get; set; }
    }
    public class pmsNewAccountDataRSModel
    {

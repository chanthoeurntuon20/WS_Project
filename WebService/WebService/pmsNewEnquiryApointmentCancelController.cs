...
{
    [BasicAuthentication]
    public class pmsNewEnquiryApointmentCancelController : ApiController
    {
        public string Post([FromUri]string p, [FromUri]string msgid, [FromBody]string json)
        {
            ...
            string ControllerName = "pmsNewEnquiryApointmentCancel";
            string FileNameForLog = msgid + "_" + ServerDate.Replace("-", "_").Replace(" ", "_").Replace(":", "_").Replace(".", "_");
            List<pmsNewEnquiryApointmentCancelRSModel> RSData = new List<pmsNewEnquiryApointmentCancelRSModel>();
            try
            {
                #region msgid
                if (ERR != "Error")
                ...
                #region read json
                pmsNewEnquiryApointmentCancelRQModel jObj = null;
                string
                ...
                if (ERR != "Error")
                {
                    try
                    {
                        jObj = JsonConvert.DeserializeObject<pmsNewEnquiryApointmentCancelRQModel>(json);
                ...
                if (ERR != "Error")
                {
                    ...
                    try
                    {
                        string sql = "exec fact_pms_apointment_new_enquiry_cancel @pms_apo_id,@CID,@comment_cancel,@create_by";
            ...
            pmsNewEnquiryApointmentCancelRSModel Header = new pmsNewEnquiryApointmentCancelRSModel();
            Header.ERR = ERR;
            Header.SMS = SMS;
...
public class pmsNewEnquiryApointmentCancelRQModel
{
...
public class pmsNewEnquiryApointmentCancelRSModel
{

...
{
    [BasicAuthentication]
    public class pmsNewEnquiryApointmentMeetController : ApiController
    {
        ...
        public string Post([FromUri]string p, [FromUri]string msgid, [FromBody]string json)
        {
            ...
            string ControllerName = "pmsNewEnquiryApointmentMeet";
            string FileNameForLog = msgid + "_" + ServerDate.Replace("-", "_").Replace(" ", "_").Replace(":", "_").Replace(".", "_");
            List<pmsNewEnquiryApointmentMeetRSModel> RSData = new List<pmsNewEnquiryApointmentMeetRSModel>();
            try
            {
                #region msgid
                if (ERR != "Error")
                ...
                #region read json
                pmsNewEnquiryApointmentMeetRQModel jObj = null;
                string
                ...
                if (ERR != "Error")
                {
                    try
                    {
                        jObj = JsonConvert.DeserializeObject<pmsNewEnquiryApointmentMeetRQModel>(json);
                ...
                if (ERR != "Error")
                {
                    ...
                    try
                    {
                        string sql = "exec fact_pms_apointment_new_enquiry_meet @pms_apo_id,@CID,@pms_result,@pms_reason,@comment_meet,@create_by";
            ...
            pmsNewEnquiryApointmentMeetRSModel Header = new pmsNewEnquiryApointmentMeetRSModel();
            Header.ERR = ERR;
            Header.SMS = SMS;
...
public class pmsNewEnquiryApointmentMeetRQModel
{
...
public class pmsNewEnquiryApointmentMeetRSModel
{

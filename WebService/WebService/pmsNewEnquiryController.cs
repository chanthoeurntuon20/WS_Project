...
{
    [BasicAuthentication]
    public class pmsNewEnquiryController : ApiController
    {
        ...
        {//json={"search_type":"CID","search_data":"1614328","hit_date_from":"2020-08-21","hit_date_to":"2020-08-21","co_T24":"1234"}
            ...
            string ControllerName = "pmsNewEnquiry";
            string FileNameForLog = msgid + "_" + ServerDate.Replace("-", "_").Replace(" ", "_").Replace(":", "_").Replace(".", "_");
            List<pmsNewEnquiryRSModel> RSData = new List<pmsNewEnquiryRSModel>();
            try
            {
                #region msgid
                if (ERR != "Error")
                ...
                #region read json
                pmsNewEnquiryRQModel jObj = null;
                string search_type = "", search_data="", hit_date_from="", hit_date_to="", co_T24="";
                if (ERR != "Error")
                {
                    try
                    {
                        jObj = JsonConvert.DeserializeObject<pmsNewEnquiryRQModel>(json);
                        //pms_sp = jObj.pms_sp;
                ...
                if (ERR != "Error")
                {
                    ...
                    try {
                        string sql = "exec pmsamk_new_enquiry @search_type=@search_type,@search_data=@search_data,@hit_date_from=@hit_date_from,@hit_date_to=@hit_date_to,@co_T24=@co_T24";
                        ...
                        pmsNewEnquiryRSModel ListHeader = new pmsNewEnquiryRSModel();
                        ListHeader.ERR = ERR;
                        ListHeader.SMS = SMS;
                        ListHeader.ERRCode = ERRCode;
                        List<pmsNewEnquiryDataRSModel> DataList = new List<pmsNewEnquiryDataRSModel>();
                        for (int i = 0; i < dt.Rows.Count; i++)
                        {
                            #region params
                            pmsNewEnquiryDataRSModel data = new pmsNewEnquiryDataRSModel();
                            ...
                            data.number_of_enquiry = dt.Rows[i]["number_of_enquiry"].ToString();
                            DataList.Add(data);
                            #endregion params
                        }
            ...
            if (ERR == "Error")
            {
                pmsNewEnquiryRSModel CustHeader = new pmsNewEnquiryRSModel();
                CustHeader.ERR = ERR;
                CustHeader.SMS = SMS;
    ...
    #region model    
    public class pmsNewEnquiryRQModel
    {
    ...
    public class pmsNewEnquiryRSModel
    {
        public string ERR { get; set; }
        public string SMS { get; set; }
        public string ERRCode { get; set; }
        public List<pmsNewEnquiryDataRSModel> Data { get; set; }
    }
    public class pmsNewEnquiryDataRSModel
    {
